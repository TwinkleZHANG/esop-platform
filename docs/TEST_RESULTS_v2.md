...
> 以下缺陷在测试报告生成后已修复，修复代码已合并至 main 分支。

| 编号 | 严重等级 | 描述 | 修复 commit | 状态 |
|------|---------|------|------------|------|
| BUG-003 | S1 | 缺估值的 RSU 归属补录估值后不会补生成税务事件 | feat/fix-test-v2-bugs | ✅ 已修复 |
| BUG-002 | S1 | Closing 状态下行权窗口期不可二次修改 | feat/fix-test-v2-bugs | ✅ 已修复 |
| BUG-001 | S2 | 用工主体删除功能未实现 | feat/fix-test-v2-bugs | ✅ 已修复 |

**额外加固：**

| 项目 | 描述 | 状态 |
|------|------|------|
| CLARIFY-002 | 唯一超管不可被降级，增加硬约束 | ✅ 已加固 |
| CLARIFY-008 | 同日多条估值取最新创建的 | ✅ 已加固 |
| CLARIFY-003 | 离职员工始终可登录，PRD 已明确 | ✅ PRD 已澄清 |
| CLARIFY-007 | 正常关闭 Closing 时保留待审批申请 | ✅ PRD 已澄清 |

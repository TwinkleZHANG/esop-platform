...
员工提交行权申请时，系统校验以下条件：
1. 申请数量 ≤ operableOptions
2. 当前日期 ≤ 行权截止日（exerciseDeadline）
3. 若 Grant 为 Closing 状态，当前日期 ≤ exerciseWindowDeadline

以上条件全部满足才可提交。若行权截止日已过，该 Grant 的「申请」按钮隐藏。
...
- **授予管理员**创建计划
- 授予管理员点「查看」→ 可「编辑」或「删除」或「返回列表」（仅「审批中」状态可编辑）
- 审批管理员点「查看」→ 可「审批通过」或「返回列表」
- **只有「已通过」状态的计划才能在授予管理中被引用**
- 审批中的计划可以删除（删除前校验：该计划下不能有任何授予记录，即使是 Draft 状态）
- 已通过的计划不可删除
...
- 离职员工账号始终保持可登录状态，可查看所有历史记录。若仍有 operableShares > 0，可继续对实股提交 post-settlement 申请；若 Closing 窗口期内仍有 operableOptions > 0，可继续提交行权申请。当所有 Grant 的 operableShares 和 operableOptions 均归零后，员工端仅可查看历史数据，不可提交新申请。
...
- 顶部：「创建授予」按钮
- **待审批提醒区**：如果有员工新提交的申请（行权/转让/售出/回购/兑现），在列表最上方高亮显示对应的授予记录，提醒管理员点击查看并审批。待审批提醒区最多显示 3 条，超出时按「下一页」显示后面的授予信息。
...
| 10  | 行权期（年）  | 条件必填  | 下拉框单选 | 4/5/6/7/8/9/10 年或自定义，必须大于归属年限；从授予计划开始日期起算的行权有效期；Option 必填 |
| 11 | 员工签署协议ID | 选填     | 文本输入 | 创建时选填（草稿阶段可暂不填写），但 Draft → Granted 时变为必填。如创建时未填，状态变更时必须补填。 |

**【归属计划】**

| #  | 字段名   | 必填 | 类型     | 说明                               |
| -- | -------- | ---- | -------- | ---------------------------------- |
| 12 | 归属年限 | 必填 | 下拉单选 | 1/2/3/4/5年（可添加自定义年限）    |
| 13 | 悬崖期   | 必填 | 下拉单选 | 0/6/12/18/24月（可添加自定义月限） |
| 14 | 归属频率 | 必选 | 下拉单选 | 按月 / 按年                        |
...
- 申请记录展示该 Grant 下所有历史申请（包括已批准、已驳回、已关闭），不仅是待审批的
- 增加「操作人」列：待审批显示"-"，已批准/已驳回显示审批管理员姓名
- 有待审批申请时，申请记录板块自动展开
...
- Draft 状态下，授予管理员可「编辑」或「删除」，审批管理员可修改状态到「授予granted」
- 删除仅 Draft 状态可用，删除后释放额度回计划池
- 其他状态「查看」只有「返回列表」或修改状态为「关闭」
- 审批管理员在详情页审批员工申请
- **关闭 Grant**：管理员可手动触发，需填写关闭原因。
  - RSU 或 Option `operableOptions == 0` → 直接 Closed
  - Option `operableOptions > 0`（正常关闭）→ 进入 Closing，按原行权期保留行权权利，行权期到期后系统自动 Closed
- RSU 或 Option `operableOptions == 0` → 直接 Closed
...
- **正常关闭（非离职）**：管理员关闭 Option 授予时，不设置窗口期。已归属未行权的期权按原行权期保留行权权利。PENDING 归属 → CLOSED，Grant 进入 Closing（如有 operableOptions > 0）或直接 Closed。
- 正常关闭进入 Closing 时，已有的待审批申请保留，审批管理员可继续审批通过或驳回。仅在行权期到期或窗口期到期时，系统才自动将剩余待审批申请关闭。
- **离职关闭**：设置窗口期。实际行权截止日 = min(行权截止日, 今天 + 窗口期天数)。员工端倒计时按实际行权截止日计算。
...
|------|------|------|
| 行权价 | 每股行权价格 | 必填，> 0 |
| 行权期（年） | 从授予计划开始日期起算的行权有效期 | 必填，下拉选择 4/5/6/7/8/9/10 年或自定义，必须 > 归属年限 |
| 行权截止日 | 系统自动计算：授予计划开始日期 + 行权期 | 自动生成，不可编辑 |
...
- 列表页顶部增加待处理提醒区（与授予管理一致），显示「已上传凭证」状态的事件，每页 3 条
- 列表和详情展示关联的计划标题和激励类型（RSU/Option）
...
- 侧边栏「税务记录」菜单项显示角标：当有「待缴款」状态的税务事件时显示数量

**行权期到期提醒（仅 Option）：**

当 Option 授予距行权截止日不足 90 天时，在员工端顶部显示提醒横幅：
- 「您有一条期权授予的行权期即将到期，已归属未行权期权：X 份，行权截止日：YYYY-MM-DD，剩余 N 天」
- 当 operableOptions 变为 0（全部行权完毕）时提醒消失
- 行权截止日当天显示「今日是最后行权日」
- 行权截止日过后显示「行权期已到期，未行权期权已失效」+ 「确认」按钮，员工点击后提醒消失

**提醒优先级（同一 Grant 多种提醒时）：**
- 若同时有窗口期和行权期，按实际截止日较早的那个显示提醒
- 即 min(exerciseWindowDeadline, exerciseDeadline)，与行权申请校验逻辑一致
...
- 列表每条记录可点击，进入授予详情页
- 授予详情页展示：基本信息、归属计划、税务事件、状态变更日志、申请记录（与管理端一致，但不显示编辑/审批按钮）
...
| 计划标题 | 激励类型 | 税务类型 | 具体操作 | 触发日期 | 数量 | 状态 | 操作 |
| ------- | ------ | ------- | ------- | ------ | ---- | --- | --- |
...
model Grant {
  ...
  exercisePeriodYears       Int?         // 行权期年数（仅 Option，从 vestingStartDate 起算）
  exerciseDeadline          DateTime?    // 行权截止日（系统自动计算：vestingStartDate + exercisePeriodYears）
  createdAt                 DateTime     @default(now())
  updatedAt                 DateTime     @updatedAt
...
- 员工无法访问 /admin/* 路由
- 管理员可访问 /employee/* 路由（切换到员工视图查看自己的股权信息）
...
| 员工状态 → 离职 | ① 所有待审批申请自动关闭（状态 → 已关闭）<br>② RSU 非 All Settled 的 Grant → Closed<br>③ Option 非 All Settled 的 Grant：operableOptions > 0 → Closing，设置窗口期，实际行权截止日 = min(原行权截止日, 今天 + 窗口期天数)；operableOptions == 0 → Closed<br>④ 已批准但税务未确定的事件，需管理员手动决定 |
...
- 所有列表默认每页 10 条，按创建时间倒序排列
- 支持翻页
- 搜索和筛选结果同样分页
- 所有列表页支持日期范围筛选
- 筛选状态持久化：搜索词、筛选条件、页码、日期范围写入 URL query params，从详情页返回列表时自动恢复
...
- **分页组件响应式**：宽屏一行显示，窄屏自动换为两行（统计信息一行，翻页操作一行）
- **筛选栏响应式**：窄屏时搜索框、筛选下拉、日期范围各自独占一行；日期范围标签和输入框分两行
- **提醒区响应式**：每条提醒内容单行横滚；标题与翻页操作窄屏时分两行
- **详情页 vs 编辑页分离**：所有实体的「查看」进入只读详情页，详情页内有「编辑」按钮切换为编辑模式，不可直接进入编辑
- **操作按钮样式**：所有可点击按钮必须有明显视觉样式（背景色或边框 + hover 效果）。主要操作用主色背景 + 白色文字，次要操作用灰色边框。禁止纯文字链接样式充当按钮
...
扫描所有非终态 Option Grant：
- 若当前日期 > exerciseDeadline：
  - operableOptions → 0
  - VESTED / PARTIALLY_SETTLED 归属记录 → CLOSED
  - Grant → CLOSED（若无 operableShares）
  - PENDING 行权申请 → CLOSED
  - 释放未行权额度回计划池
  - 写 StatusChangeLog（operatorName = "系统自动触发 - 行权期到期"）
